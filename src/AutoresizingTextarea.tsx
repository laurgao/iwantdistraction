...
const AutoresizingTextarea = (props: Props) => {
    ...
    const newClassName = className + " border rounded-md w-full my-2 text-stone-900 text-opacity-70 " + paddingClass;
    return (
        <div className="relative">
            {/* @ts-ignore ref error */}
            <div {...newProps} style={style} className={newClassName} contentEditable onInput={handleChange} ref={ref} />
            <div className={"absolute top-0 left-0 text-stone-900 text-opacity-40 " + paddingClass} style={{ zIndex: -10 }}>
